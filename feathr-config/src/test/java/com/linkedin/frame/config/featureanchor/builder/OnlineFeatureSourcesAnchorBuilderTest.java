...
public class OnlineFeatureSourcesAnchorBuilderTest {
  @Mock
  ...
  public void testBuild() throws URISyntaxException {
    ...
    //trigger
    OnlineFeatureSourcesAnchorBuilder builder =
        new OnlineFeatureSourcesAnchorBuilder(derivationConfig, _derivedFeatureDependencyResolver,
            _derivedFeatureTransformationFunctionBuilder, _keyPlaceholdersBuilder);
    Anchor anchor = builder.build();

    //assertion
    assertEquals(anchor.getOnlineFeatureSourcesAnchor().getSource(), featureSources);
    assertEquals(anchor.getOnlineFeatureSourcesAnchor().getTransformationFunction(), transformationFunction);
    assertEquals(anchor.getOnlineFeatureSourcesAnchor().getKeyPlaceholders(), keyPlaceholderArray);
  }
}
