...
public abstract class AbstractConfigBuilderTest {
  ...
  public void testConfigBuilder(Namespace namespace, String configStr,
      TriFunction<Namespace, String, Config, ConfigObj> configBuilder, ConfigObj expConfigObj) {
    assertEquals(buildConfig(namespace, configStr, configBuilder), expConfigObj);
  }

  public void testConfigBuilder(Namespace namespace, String configStr,
      BiFunction<Namespace, Config, ConfigObj> configBuilder, ConfigObj expConfigObj) {
    Config config = ConfigFactory.parseString(configStr);
    ConfigObj obsConfigObj = configBuilder.apply(namespace, config);

    assertEquals(obsConfigObj, expConfigObj);
  }

  public ConfigObj buildConfig(Namespace namespace, String configStr,
      TriFunction<Namespace, String, Config, ConfigObj> configBuilder) {
    Config fullConfig = ConfigFactory.parseString(configStr);
    String configName = fullConfig.root().keySet().iterator().next();
    Config config = fullConfig.getConfig(quote(configName));
    return configBuilder.apply(namespace, configName, config);
  }
