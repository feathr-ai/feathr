...
public class FeatureDefConfSemanticValidatorTest {
  ...
  public void testGetDuplicateFeatureNames() {
    try (ConfigDataProvider provider = new ResourceConfigDataProvider("invalidSemanticsConfig/duplicate-feature.conf")) {
      ...
      Set<String> duplicateFeatureNames =
          configValidator.getDuplicateFeatureNames(featureDefConfig);

      Assert.assertEquals(duplicateFeatureNames.size(), 1);
      Assert.assertEquals(duplicateFeatureNames.toArray()[0], "member_lixSegment_isJobSeeker");
  ...
  @Test(description = "Tests non-approved extractor with parameters will throw exception.", expectedExceptions = FrameConfigException.class)
