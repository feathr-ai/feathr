...
import com.linkedin.frame.exception.ErrorLabel;
import com.linkedin.frame.exception.FrameConfigException;
...
import static com.linkedin.frame.core.configvalidator.typesafe.FeatureReachType.*;
...
class FeatureDefConfigSemanticValidator {
  ...
  // TODO(PROML-17349): Add Galene's parameterized extractors.
  private static final Set<String> ALLOWED_EXTRACTOR_WITH_PARAMETERS = ImmutableSet.of(
      "com.linkedin.frame.SampleExtractorWithParams",
      ...
      "com.linkedin.followfeed.frame.extractor.KeyTagFeatureExtractor");
  ...
  void validateApprovedExtractorWithParameters(FeatureDefConfig featureDefConfig) {
    for (Map.Entry<String, AnchorConfig> entry : featureDefConfig.getAnchorsConfig().get().getAnchors().entrySet()) {
      ...
      for (Map.Entry<String, FeatureConfig> featureEntry : anchorConfig.getFeatures().entrySet()) {
        ...
        if (featureConfig instanceof SimpleFeatureConfig && !featureConfig.getParameters().isEmpty()) {
          if (anchorConfig instanceof AnchorConfigWithExtractor) {
            ...
            if (!ALLOWED_EXTRACTOR_WITH_PARAMETERS.contains(extractor)) {
              throw new FrameConfigException(ErrorLabel.FRAME_USER_ERROR, "anchorConfig: " + anchorConfig
            ...
            throw new FrameConfigException(ErrorLabel.FRAME_USER_ERROR,
  ...
  private static Set<String> getInputFeatures(String derivedFeature,
    ...
    if (derivationConfig instanceof DerivationConfigWithExpr) {
      ...
      Matcher matcher = FeatureRef.FEATURE_NAME_PATTERN.matcher(featureExpr);
