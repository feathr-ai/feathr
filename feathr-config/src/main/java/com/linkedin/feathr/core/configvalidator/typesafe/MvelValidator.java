...
class MvelValidator {
  ...
  Map<String, String> getFeatureMvels(FeatureDefConfig featureDefConfig) {
    ...
    BiConsumer<String, AnchorConfig> consumeAnchor = (anchorName, anchorConfig) -> {
      for (Map.Entry<String, FeatureConfig> entry : anchorConfig.getFeatures().entrySet()) {
        ...
        if (featureConfig instanceof SimpleFeatureConfig) {
          featureNameToMvel.put(featureName, ((SimpleFeatureConfig) featureConfig).getFeatureName());
        } else if (featureConfig instanceof ComplexFeatureConfig) {
          ...
          if (complexFeatureConfig.getExprType() == ExprType.MVEL) {
            featureNameToMvel.put(featureName, complexFeatureConfig.getFeatureExpr());
          }
   ...
   *  won't throw exception. Instead, it will only return wrong results. One such example is PROML-3886.
   *
