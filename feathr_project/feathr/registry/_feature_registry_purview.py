...
from pyapacheatlas.core import AtlasClassification, AtlasEntity, AtlasProcess, PurviewClient, TypeCategory
...
def _to_snake(d, level: int = 0):
    ...
    return {
        _to_snake(a, level + 1): _to_snake(b, level + 1) if isinstance(b, (dict, list)) else b for a, b in d.items()
    }
...
class _PurviewRegistry(FeathrRegistry):
    ...
    def __init__(
        self,
        project_name: str,
        azure_purview_name: str,
        registry_delimiter: str,
        project_tags: Dict[str, str] = None,
        credential=None,
        config_path=None,
    ):
        ...
        self.credential = (
            DefaultAzureCredential(exclude_interactive_browser_credential=False) if credential is None else credential
        )
        self.oauth = AzCredentialWrapper(credential=self.credential)
        self.purview_client = PurviewClient(account_name=self.azure_purview_name, authentication=self.oauth)
    ...
    def _register_feathr_feature_types(self):
        ...
        type_feathr_project = EntityTypeDef(
            ...
            attributeDefs=[
                ...
                AtlasAttributeDef(name="anchor_features", typeName=TYPEDEF_ARRAY_ANCHOR, cardinality=Cardinality.SET),
                AtlasAttributeDef(
                    name="derived_features", typeName=TYPEDEF_ARRAY_DERIVED_FEATURE, cardinality=Cardinality.SET
                ),
                AtlasAttributeDef(name="tags", typeName="map<string,string>", cardinality=Cardinality.SINGLE),
            ],
            superTypes=["DataSet"],
        )
        type_feathr_sources = EntityTypeDef(
            name=TYPEDEF_SOURCE,
            attributeDefs=[
                AtlasAttributeDef(name="path", typeName="string", cardinality=Cardinality.SINGLE),
                AtlasAttributeDef(name="url", typeName="string", cardinality=Cardinality.SINGLE),
                AtlasAttributeDef(name="dbtable", typeName="string", cardinality=Cardinality.SINGLE),
                AtlasAttributeDef(name="query", typeName="string", cardinality=Cardinality.SINGLE),
                AtlasAttributeDef(name="auth", typeName="string", cardinality=Cardinality.SINGLE),
                AtlasAttributeDef(name="event_timestamp_column", typeName="string", cardinality=Cardinality.SINGLE),
                AtlasAttributeDef(name="timestamp_format", typeName="string", cardinality=Cardinality.SINGLE),
                AtlasAttributeDef(name="type", typeName="string", cardinality=Cardinality.SINGLE),
                AtlasAttributeDef(name="preprocessing", typeName="string", cardinality=Cardinality.SINGLE),
                AtlasAttributeDef(name="tags", typeName="map<string,string>", cardinality=Cardinality.SINGLE),
            ],
            superTypes=["DataSet"],
        )
        ...
        type_feathr_anchor_features = EntityTypeDef(
            ...
            attributeDefs=[
                AtlasAttributeDef(name="type", typeName="string", cardinality=Cardinality.SINGLE),
                AtlasAttributeDef(name="key", typeName="array<map<string,string>>", cardinality=Cardinality.SET),
                AtlasAttributeDef(name="transformation", typeName="map<string,string>", cardinality=Cardinality.SINGLE),
                AtlasAttributeDef(name="tags", typeName="map<string,string>", cardinality=Cardinality.SINGLE),
            ],
            superTypes=["DataSet"],
        )
        ...
        type_feathr_derived_features = EntityTypeDef(
            ...
            attributeDefs=[
                AtlasAttributeDef(name="type", typeName="string", cardinality=Cardinality.SINGLE),
                AtlasAttributeDef(
                    name="input_anchor_features", typeName=TYPEDEF_ARRAY_ANCHOR_FEATURE, cardinality=Cardinality.SET
                ),
                AtlasAttributeDef(
                    name="input_derived_features", typeName=TYPEDEF_ARRAY_DERIVED_FEATURE, cardinality=Cardinality.SET
                ),
                AtlasAttributeDef(name="key", typeName="array<map<string,string>>", cardinality=Cardinality.SET),
                AtlasAttributeDef(name="transformation", typeName="map<string,string>", cardinality=Cardinality.SINGLE),
                AtlasAttributeDef(name="tags", typeName="map<string,string>", cardinality=Cardinality.SINGLE),
        ...
        type_feathr_anchors = EntityTypeDef(
            name=TYPEDEF_ANCHOR,
            attributeDefs=[
                AtlasAttributeDef(name="source", typeName=TYPEDEF_SOURCE, cardinality=Cardinality.SINGLE),
                AtlasAttributeDef(name="features", typeName=TYPEDEF_ARRAY_ANCHOR_FEATURE, cardinality=Cardinality.SET),
                AtlasAttributeDef(name="tags", typeName="map<string,string>", cardinality=Cardinality.SINGLE),
            ],
            superTypes=["DataSet"],
        )
        ...
        def_result = self.purview_client.upload_typedefs(
            entityDefs=[
                type_feathr_anchor_features,
                type_feathr_anchors,
                type_feathr_derived_features,
                type_feathr_sources,
                type_feathr_project,
            ],
            force_update=True,
    ...
    def _parse_anchor_features(self, anchor: FeatureAnchor) -> List[AtlasEntity]:
        ...
        for anchor_feature in anchor.features:
            ...
            for individual_key in anchor_feature.key:
                key_dict = {
                    "key_column": individual_key.key_column,
                    "key_column_type": individual_key.key_column_type.value,
                    "full_name": individual_key.full_name,
                    "description": individual_key.description,
                    "key_column_alias": individual_key.key_column_alias,
                }
        ...
        for anchor in anchor_list:
            ...
            for anchor_feature_entity in anchor_feature_entities:
                lineage = AtlasProcess(
                    ...
                    typeName="Process",
                    qualified_name=self.registry_delimiter
                    + "PROCESS"
                    + self.registry_delimiter
                    + self.project_name
                    + self.registry_delimiter
    ...
    def _parse_derived_features(self, derived_features: List[DerivedFeature]) -> List[AtlasEntity]:
        ...
        for derived_feature in toposorted_derived_feature_list:
            ...
            derived_feature_entity = AtlasEntity(
                ...
                qualified_name=self.project_name + self.registry_delimiter + derived_feature.name,
                attributes={
                    "type": derived_feature.feature_type.to_feature_config(),
                    "key": key_list,
                    "input_anchor_features": [
                        f.to_json(minimum=True)
                        for f in input_feature_entity_list
                        if f.typeName == TYPEDEF_ANCHOR_FEATURE
                    ],
                    "input_derived_features": [
                        f.to_json(minimum=True)
                        for f in input_feature_entity_list
                        if f.typeName == TYPEDEF_DERIVED_FEATURE
                    ],
                    "transformation": transform_dict,
    ...
    def _parse_features_from_context(self, workspace_path: str, anchor_list, derived_feature_list):
        ...
        project_attributes = {
            "anchor_features": [s.to_json(minimum=True) for s in anchor_entities],
            "tags": self.project_tags,
        }
        # add derived feature if it's there
        if derived_feature_list:
            derived_feature_entities = self._parse_derived_features(derived_feature_list)
            project_attributes["derived_features"] = [s.to_json(minimum=True) for s in derived_feature_entities]
    ...
    def upload_single_entity_to_purview(self, entity: Union[AtlasEntity, AtlasProcess]):
            ...
            if j["typeName"] == response["typeName"]:
                if j["typeName"] == "Process":
                    if response["attributes"]["qualifiedName"] != j["attributes"]["qualifiedName"]:
                        raise RuntimeError(
                            "The requested entity %s conflicts with the existing entity in PurView"
                            % j["attributes"]["qualifiedName"]
                        )
                else:
                    if "type" in response["attributes"] and response["typeName"] in (
        ...
        try:
            ...
            if "PreConditionCheckFailed" in e.args[0]:
                entity.guid = self.purview_client.get_entity(
                    qualifiedName=entity.qualifiedName, typeName=entity.typeName
                )["entities"][0]["guid"]
    ...
    def _create_source(self, s: Source) -> UUID:
        ...
        """
        project_entity = self.purview_client.get_entity(
            qualifiedName=self.project_name, typeName=TYPEDEF_FEATHR_PROJECT
        )["entities"][0]
    ...
    def _create_anchor(self, s: FeatureAnchor) -> UUID:
        ...
        source_entity = self.purview_client.get_entity(
            qualifiedName=self.registry_delimiter.join([self.project_name, s.source.name]), typeName=TYPEDEF_SOURCE
        )["entities"][0]
        ...
        anchor_consumes_source_relation = self._generate_relation_pairs(anchor_entity, source_entity, RELATION_CONSUMES)
        [
            self.upload_single_entity_to_purview(x)
            for x in project_contains_anchor_relation + anchor_consumes_source_relation
        ]
    ...
    def _create_anchor_feature(self, anchor_id: str, source: Source, s: Feature) -> UUID:
        ...
        anchor_entity = self.purview_client.get_entity(anchor_id)["entities"][0]
        source_entity = self.purview_client.get_entity(
            qualifiedName=self.registry_delimiter.join([self.project_name, source.name]), typeName=TYPEDEF_SOURCE
        )["entities"][0]

        attrs = feature_to_def(s)
        attrs["type"] = attrs["featureType"]
        qualified_name = self.registry_delimiter.join(
            [self.project_name, anchor_entity["attributes"]["name"], attrs["name"]]
        )
        ...
        [
            self.upload_single_entity_to_purview(x)
            for x in project_contains_feature_relation
    ...
    def _create_derived_feature(self, s: DerivedFeature) -> UUID:
        ...
        [self.upload_single_entity_to_purview(x) for x in feature_project_contain_belong_pairs + consume_produce_pairs]
    ...
    def register_features(
        ...
        for anchor in anchor_list:
            ...
            for feature in anchor.features:
                if not hasattr(feature, "_registry_id"):
                    feature._registry_id = self._create_anchor_feature(anchor._registry_id, anchor.source, feature)
        ...
        logger.info("Finished registering features.")
    ...
    def _delete_all_feathr_entities(self):
        ...
        while True:
            result = self.purview_client.discovery.query("feathr", limit=batch_delete_size)
    ...
    def list_registered_features(self, project_name: str, limit=1000, starting_offset=0) -> List[Dict[str, str]]:
        ...
        query_filter = {
            "and": [
                {"or": [{"entityType": TYPEDEF_DERIVED_FEATURE}, {"entityType": TYPEDEF_ANCHOR_FEATURE}]},
                {
    ...
    def get_feature_id(self, qualifiedName, type: str):
        ...
        # see syntax here: https://docs.microsoft.com/en-us/rest/api/purview/catalogdataplane/discovery/query#discovery_query_andornested
        query_filter = {"attributeName": "qualifiedName", "operator": "eq", "attributeValue": qualifiedName}
    ...
    def _list_registered_entities_with_details(
        self,
        project_name: str,
        ...
        result_entities = self.purview_client.discovery.search_entities(
            query=None, search_filter=query_filter, limit=limit
        )
        ...
        entity_res = (
            ...
            else self.purview_client.get_entity(guid=guid_list)["entities"]
        )
        return entity_res
    ...
    def get_features_from_registry(self, project_name: str) -> Tuple[List[FeatureAnchor], List[DerivedFeature]]:
        ...
        """
        project_entity = self.purview_client.get_entity(qualifiedName=project_name, typeName=TYPEDEF_FEATHR_PROJECT)[
            "entities"
        ][0]

        single_direction_process = [
            entity
            for _, entity in self.purview_client.get_entity_lineage(project_entity["guid"])["guidEntityMap"].items()
        ...
        contain_relations = [
            x["displayText"].split(" to ")
            for x in single_direction_process
            if x["attributes"]["qualifiedName"].startswith(RELATION_CONTAINS)
        ]

        consume_relations = [
            x["displayText"].split(" to ")
            for x in single_direction_process
            if x["attributes"]["qualifiedName"].startswith(RELATION_CONSUMES)
        ]

        entities_under_project = [
            self.purview_client.get_entity(x[1])["entities"][0]
            for x in contain_relations
            if x[0] == project_entity["guid"]
        ]
        ...
        for derived_feature_entity in [x for x in entities_under_project if x["typeName"] == TYPEDEF_DERIVED_FEATURE]:
            ...
            for key in derived_feature_entity["attributes"]["key"]:
                derived_feature_key_list.append(
                    TypedKey(
                        key_column=key["keyColumn"],
                        key_column_type=key["keyColumnType"],
                        full_name=key["fullName"],
                        description=key["description"],
                        key_column_alias=key["keyColumnAlias"],
                    )
                )
            ...
            def search_for_input_feature(elem, full_relations, full_entities):
                ...
                input_features = [
                    ...
                    if x["typeName"] == TYPEDEF_ANCHOR_FEATURE or x["typeName"] == TYPEDEF_DERIVED_FEATURE
                ]
                result_features = []
                for feature_entity in input_features:
                    ...
                    result_features.append(
                        Feature(
                            name=feature_entity["attributes"]["name"],
                            feature_type=self._get_feature_type_from_hocon(
                                feature_entity["attributes"]["type"]
                            ),  # stored as a hocon string, can be parsed using pyhocon
                            transform=self._get_transformation_from_dict(
                                feature_entity["attributes"]["transformation"]
                            ),  # transform attributes are stored in a dict fashion , can be put in a WindowAggTransformation
            ...
            derived_feature_list.append(
                DerivedFeature(
                    name=derived_feature_entity["attributes"]["name"],
                    feature_type=self._get_feature_type_from_hocon(derived_feature_entity["attributes"]["type"]),
                    transform=self._get_transformation_from_dict(
                        derived_feature_entity["attributes"]["transformation"]
                    ),
        ...
        for anchor_entity in [x for x in entities_under_project if x["typeName"] == TYPEDEF_ANCHOR]:
            consume_items_under_anchor = [
                entities_dict[x[1]] for x in consume_relations if x[0] == anchor_entity["guid"]
            ]
            source_entity = [x for x in consume_items_under_anchor if x["typeName"] == TYPEDEF_SOURCE][0]

            contain_items_under_anchor = [
                entities_dict[x[1]] for x in contain_relations if x[0] == anchor_entity["guid"]
            ]
            anchor_features_guid = [
                x["guid"] for x in contain_items_under_anchor if x["typeName"] == TYPEDEF_ANCHOR_FEATURE
            ]

            anchor_list.append(
                FeatureAnchor(
                    name=anchor_entity["attributes"]["name"],
                    source=HdfsSource(
                    ...
                    features=self._get_features_by_guid_or_entities(
                        guid_list=anchor_features_guid, entity_list=entities_under_project
                    ),
    ...
    def _get_feature_type_from_hocon(self, input_str: str) -> FeatureType:
        ...
        dimensionType = (
            conf.get_string("dimensionType") if "dimensionType" in conf else conf.get_string("type.dimensionType")
        )
    ...
    def _get_features_by_guid_or_entities(self, guid_list, entity_list) -> List[FeatureAnchor]:
        ...
        if not entity_list:
            ...
            if len(feature_entities) != len(guid_list):
                raise RuntimeError(
                    "Number of `feature_entities` is less than provided GUID list for search. The project might be broken."
                )
