...
class _FeathrSynapseJobLauncher(SparkJobLauncher):
    ...
    def upload_or_get_cloud_path(self, local_path_or_cloud_src_path: str, tar_dir_path: Optional[str] = None):
        ...
        if local_path_or_cloud_src_path.startswith("abfs") or local_path_or_cloud_src_path.startswith("wasb"):
            ...
            logger.info("{} is uploaded to location: {}", local_path_or_cloud_src_path, tar_dir_path)
            return tar_dir_path
        ...
        logger.info("{} is uploaded to location: {}", local_path_or_cloud_src_path, res_path)
        return res_path
    ...
    def submit_feathr_job(
        ...
        self.current_job_info = self._api.create_spark_batch_job(
            job_name=job_name,
    ...
    def wait_for_completion(self, timeout_seconds: Optional[float]) -> bool:
        ...
        while (timeout_seconds is None) or (time.time() - start_time < timeout_seconds):
            ...
            if status in {LivyStates.SUCCESS.value}:
                ...
                logger.error(
                    "The size of the whole error log is: {}. The logs might be truncated in some cases (such as in Visual Studio Code) so only the top a few lines of the error message is displayed. If you cannot see the whole log, you may want to extend the setting for output size limit.",
                    len(error_msg),
                )
                return False
            else:
...
class _SynapseJobRunner(object):
    ...
    def __init__(self, synapse_dev_url, spark_pool_name, credential=None, executor_size="Small", executors=2):
        ...
        self.client = SparkClient(credential=credential, endpoint=synapse_dev_url, spark_pool_name=spark_pool_name)
    ...
    def get_driver_log(self, job_id) -> str:
        ...
        url = "%s/sparkhistory/api/v1/sparkpools/%s/livyid/%s/applications/%s/driverlog/stderr/?isDownload=true" % (
            self._synapse_dev_url,
            self._spark_pool_name,
            job_id,
            app_id,
        )
...
class _DataLakeFiler(object):
    ...
    def __init__(self, datalake_dir, credential=None):
        ...
        self.file_system_client = DataLakeServiceClient(
            credential=credential, account_url=account_url
    ...
    def upload_file_to_workdir(
        self,
        src_file_path: str,
        tar_dir_path: Optional[str] = "",
        tar_dir_client: Optional[DataLakeDirectoryClient] = None,
    ) -> str:
        """
    ...
    def upload_file(
        self, src_file_path, tar_dir_path: Optional[str] = "", tar_dir_client: Optional[DataLakeDirectoryClient] = None
    ) -> str:
        file_name = basename(src_file_path)
        logger.info("Uploading file {}", file_name)
        # TODO: add handling for only tar_dir_client or tar_dir_path is provided
        file_client = (
            self.dir_client.create_file(file_name) if tar_dir_client is None else tar_dir_client.create_file(file_name)
        )
    ...
    def download_file(self, target_adls_directory: str, local_dir_cache: str, file_name: str = None):
        ...
        if file_name is not None:
            ...
            logger.info("Finish downloading file {} from {} to {}.", file_name, target_adls_directory, local_dir_cache)
            return
        ...
        result_paths = [
            ...
            for file_path in self.file_system_client.get_paths(path=parse_result.path, recursive=False)
        ...
        result_folders = [
            file_path.name
            for file_path in self.file_system_client.get_paths(path=parse_result.path)
        ...
        for folder in result_folders:
            ...
            file_in_folder = [
                os.path.join(folder_name, basename(file_path.name))
                for file_path in self.file_system_client.get_paths(path=folder, recursive=False)
                if not file_path.is_directory
            ]
            local_paths = [os.path.join(local_dir_cache, file_name) for file_name in file_in_folder]
    ...
    def _dir_exists(self, dir_path: str):
        ...
        dir_client = self.file_system_client.get_directory_client("/".join(datalake_path_split[3:]))
