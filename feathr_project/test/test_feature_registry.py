...
from feathr import FeatureQuery, ObservationSettings, TypedKey, ValueType
...
class FeatureRegistryTests(unittest.TestCase):
        ...
        for config_path in config_paths:
            with self.subTest(config_path=config_path):
                ...
                if client.spark_runtime == "databricks":
                    output_path = "".join(
                        ["dbfs:/feathrazure_cijob", "_", str(now.minute), "_", str(now.second), ".parquet"]
                    )
                else:
                    output_path = "".join(
                        [
                ...
                client: FeathrClient = registry_test_setup_for_409(
                    os.path.join(test_workspace_dir, config_path), client.project_name
                )
    ...
    def test_get_feature_from_registry(self):
        ...
        inputs = registry.search_input_anchor_features(
            ["derived_feature_with_multiple_input_anchors"],
            entity_array_to_dict(anchors + [derived_feature_with_multiple_inputs]),
        )
        assert len(inputs) == 2
        ...
        inputs = registry.search_input_anchor_features(
            ["hierarchical_derived_feature"],
            entity_array_to_dict(anchors + [derived_feature_with_multiple_inputs, hierarchical_derived_feature]),
        )
        assert len(inputs) == 3
        assert "input_anchorA" in inputs and "input_anchorB" in inputs and "input_anchorC" in inputs

    @pytest.mark.skip(
        reason="Add back get_features is not supported in feature registry for now and needs further discussion"
    )
