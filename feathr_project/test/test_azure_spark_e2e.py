...
def test_feathr_online_store_agg_features():
    ...
    res = client.get_online_features(online_test_table, ["81", "254"], ["f_location_avg_fare", "f_location_max_fare"])
    ...
    res = client.multi_get_online_features(
        online_test_table, [["81", "254"], ["25", "42"]], ["f_location_avg_fare", "f_location_max_fare"]
    )
...
def test_feathr_get_offline_features():
    ...
    with runner.isolated_filesystem():
        runner.invoke(init, [])
        client = basic_test_setup("./feathr_user_workspace/feathr_config.yaml")
        ...
        feature_query = FeatureQuery(feature_list=["f_location_avg_fare", "f_trip_time_rounded"], key=location_id)
...
def test_feathr_materialize_to_cosmosdb():
    ...
    sink = CosmosDbSink(
        name="cosmos1",
        endpoint="https://feathrazuretest3-cosmosdb.documents.azure.com:443/",
        database="feathr",
        container=container,
    )
    settings = MaterializationSettings(
