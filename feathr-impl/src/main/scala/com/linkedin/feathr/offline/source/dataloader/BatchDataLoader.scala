  ...
  def loadDataFrameWithRetry(dataIOParameters: Map[String, String], jobConf: JobConf, retry: Int): DataFrame = {
    ...
    try {
      ...
      case error: Exception =>
        log.error(s"Loading ${location} failed with error ${error.toString}, will try load as csv.")
        try {
          ...
          case e: Exception =>
            ...
            if (retry > 0) {
              ...
              throw new FeathrInputDataException(ErrorLabel.FEATHR_USER_ERROR, s"Failed to load ${dataPath} after ${initialNumOfRetries} retries" +
                s" and retry time of ${retryWaitTime}ms. Error: ${error.toString} and ${e.toString}")
            }
            else {
              throw new FeathrInputDataException(ErrorLabel.FEATHR_USER_ERROR, s"Failed to load ${dataPath} with Error: ${error.toString} and ${e.toString}")
            } 
        }
    }
  }
