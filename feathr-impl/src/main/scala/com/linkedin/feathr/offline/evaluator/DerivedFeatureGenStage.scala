 ...
 * @param featureDataFrame base DataFrame.
 * @param joinKeys         columns of DataFrame used for joins.
 * @param featureNames     evaluated features on the DataFrame.
 */
...
private[offline] class DerivedFeatureGenStage(featureGroups: FeatureGroups, logicalPlan: MultiStageJoinPlan, derivedFeatureUtils: DerivedFeatureEvaluator)
   ...
   * the dependent features required for computation are available on a single DataFrame.
  ...
  override def evaluate(features: Seq[String], keyTags: Seq[Int], context: FeatureDataWithJoinKeys): FeatureDataWithJoinKeys = {
    ...
    featureToDerivationMap.foldLeft(context)((accumulator: FeatureDataWithJoinKeys, currFeatureDerivation) => {
      ...
      val columnRenamedDf = dropFeathrTagsAndRenameColumn(derivedFeatureDataFrame.df, featureColumnName)
      ...
      accumulator ++ updatedFeaturesOnDf.map(f => f -> (offline.FeatureDataFrame(columnRenamedDf, updatedFeatureTypeMap), joinKeys)).toMap
    })
  }
   ...
   * In such cases, the DataFrames are joined so that the dependent features are available on a single DataFrame.
    ...
    if (consumedFeatures.forall(featuresGroupedByDf(headFeatureDataFrame.df).contains)) {
      ...
      consumedFeatures.tail.foldLeft(initialBaseDataFrame)((accumulator: BaseDataFrameMetadata, consumedFeature) => {
        ...
        BaseDataFrameMetadata(
          // merge feature type mapping for features joined to the DataFrame.
   ...
   * This helper method
   */
  private def dropFeathrTagsAndRenameColumn(df: DataFrame, featureName: String): DataFrame = {
