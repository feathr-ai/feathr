...
private[offline] class AnchoredFeatureJoinStep(
  ...
  override def joinFeatures(features: Seq[ErasedEntityTaggedFeature], input: AnchorJoinStepInput)(
    ...
    val withMissingFeaturesSubstituted = substituteDefaultsForDataMissingFeatures()
    ...
    updatedObs = substituteDefaults(updatedObsDf, missingAnchoredFeatures.keys.toSeq, defaults, featureTypes, ctx.sparkSession, (s: String) => s"${FEATURE_NAME_PREFIX}$s")
    updatedObs = missingAnchoredFeatures.keys.foldLeft(updatedObs) { (updatedObs, featureName) =>
      val keyTags = ctx.logicalPlan.joinStages.filter(kv => kv._2.contains(featureName)).head._1
      val keyStr = keyTags.map(ctx.logicalPlan.keyTagIntsToStrings).toList
      updatedObs.withColumn(DataFrameColName.genFeatureColumnName(FEATURE_NAME_PREFIX + featureName, Some(keyStr)),
        col(DataFrameColName.genFeatureColumnName(FEATURE_NAME_PREFIX + featureName)))
    }
    missingAnchoredFeatures.map(x => x._2.featureAnchor.sourceKeyExtractor.getKeyColumnNames())
    ...
    val joinOutput = joinStages
      .foldLeft(FeatureDataFrame(updatedObs, Map.empty[String, FeatureTypeConfig]))((accFeatureDataFrame, joinStage) => {
