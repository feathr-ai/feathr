...
private[offline] object EventNodeEvaluator {
  def processEventNode(ss: SparkSession, dataSourceNode: com.linkedin.feathr.compute.DataSource, timeRange: Option[DateTimeInterval]): NodeContext = {
    ...
    val dataSourceAccessor = DataSourceAccessor(ss, source, timeRange, None, failOnMissingPartition = false, needTimestampColumn)
