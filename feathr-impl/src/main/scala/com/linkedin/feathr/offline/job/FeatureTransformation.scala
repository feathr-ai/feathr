...
private[offline] object FeatureTransformation {
  ...
  def getFeatureJoinKey(sourceKeyExtractor: SourceKeyExtractor, withKeyColumnRDD: RDD[_]): Seq[String] = {
    if (!sourceKeyExtractor.isInstanceOf[SpecificRecordSourceKeyExtractor] && !sourceKeyExtractor.isInstanceOf[MVELSourceKeyExtractor]) {
      throw new FeathrFeatureTransformationException(
        ErrorLabel.FEATHR_USER_ERROR,
        s"Cannot call apply ${sourceKeyExtractor} on RDD as it's expecting SpecificRecordSourceKeyExtractor or MVELSourceKeyExtractor.")
    }
