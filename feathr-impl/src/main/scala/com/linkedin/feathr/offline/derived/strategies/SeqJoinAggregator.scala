...
private[offline] object SeqJoinAggregator {
  ...
  private[feathr] def explodeLeftJoinKey(ss: SparkSession, inputDF: DataFrame, joinKeys: Seq[String], seqJoinFeatureName: String): (Seq[String], DataFrame) = {
    // isSeqJoinArrayExplodeEnabled flag is controlled "spark.frame.seq.join.array.explode.enabled" config.
    // This is a hidden config used by FRAME DEV ONLY. This knob is required for performance tuning.
  ...
  /**
   * Apply Union aggregation for SeqJoin. For more information on the union aggregation rules see the wiki at
   * https://iwww.corp.linkedin.com/wiki/cf/display/ENGS/Sequential+Join+Features+in+Frame
