...
class TestDataSource extends TestFeathr {
  ...
  def testGetFactDataAsDataframeDaily(): Unit = {
    ...
    val timeRange1 = OfflineDateTimeUtils.getFactDataTimeRange(
      ...
      Duration.ofDays(1),
      simulateTimeDelays)
    val anchorDF1 = DataSourceAccessor(ss=ss, source=mockSource, dateIntervalOpt=Some(timeRange1), expectDatumType=None, failOnMissingPartition = false, dataPathHandlers=List()).get.get()
    ...
    val timeRange2 = OfflineDateTimeUtils.getFactDataTimeRange(
      ...
      Duration.ofDays(3),
      simulateTimeDelays)
    val anchorDF2 = DataSourceAccessor(ss=ss, source=mockSource, dateIntervalOpt=Some(timeRange2), expectDatumType=None, failOnMissingPartition = false, dataPathHandlers=List()).get.get()
