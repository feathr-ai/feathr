...
public class ComputeGraphs {
  ...
  public static ComputeGraph merge(Collection<ComputeGraph> inputGraphs) {
    ...
    inputGraphs.forEach(inputGraph -> {
      ...
      inputGraph.getFeatureNames().forEach((featureName, featureDefintion) -> {
        builder.addFeatureName(featureName, featureDefintion.setNodeId(featureDefintion.getNodeId() + offset));
  ...
  public static ComputeGraph removeRedundancies(ComputeGraph inputGraph) throws CloneNotSupportedException {
    ...
    FeatureDefintionMap featureNameMap = inputGraph.getFeatureNames();
    ...
    Map<AnyNode, FeatureDefintion> standardizedNodes = new HashMap<>();
    ...
    while (!deque.isEmpty()) {
      ...
      if (!unfinishedDependencies.isEmpty()) {
        ...
        FeatureDefintion standardizedFd = standardizedNodes.get(thisNode);
        if (standardizedFd != null) {
          ...
          FeatureDefintion copyFeatureDefintion = standardizedNodes.get(thisNode).copy();
          whoDependsOnMeIndex.getOrDefault(thisNodeId, Collections.emptySet()).forEach(nodeWhoDependsOnMe ->
              Dependencies.remapDependencies(nodes.get(nodeWhoDependsOnMe),
                  ...
                  id -> id == thisNodeId ? standardizedFd.getNodeId() : id));
          ...
          featureDependencyIndex.getOrDefault(thisNodeId, Collections.emptySet()).forEach(featureThatPointsToMe ->
              featureNameMap.put(featureThatPointsToMe, copyFeatureDefintion.setNodeId(standardizedFd.getNodeId())));
        } else {
          // 3. If I DON'T have a twin in the standardized set, then put myself into the standardized set.
          standardizedNodes.put(thisNode, new FeatureDefintion().setNodeId(thisNodeId));
        }
    ...
    standardizedNodes.forEach((node, featureDefintion) -> PegasusUtils.setNodeId(node, featureDefintion.getNodeId()));
  ...
  private static ComputeGraph removeExternalNodesForFeaturesDefinedInThisGraph(ComputeGraph inputGraph) {
    ...
    for (int id = 0; id < inputGraph.getNodes().size(); id++) {
      ...
      if (node.isExternal()) {
        FeatureDefintion featureDefintion = inputGraph.getFeatureNames().get(node.getExternal().getName());
        if (featureDefintion != null) {
          // "any node who depends on me, should actually depend on that other node instead"
          externalNodeRemappedIds.put(id, featureDefintion.getNodeId());
        }
      }
    }
   ...
   */
  static ComputeGraph reindexNodes(Collection<AnyNode> nodes, Map<String, FeatureDefintion> featureNames) {
    ...
    featureNames.forEach((featureName, featureDefintion) -> {
      try {
        builder.addFeatureName(featureName, featureDefintion.copy()
            .setNodeId(remap.apply(featureDefintion.getNodeId())));
  ...
  static Map<Integer, Set<String>> getReverseFeatureDependencyIndex(ComputeGraph graph) {
    ...
    graph.getFeatureNames().forEach((featureName, featureDefintion) -> {
      Set<String> dependentFeatures = reverseDependencies.computeIfAbsent(featureDefintion.getNodeId(), x -> new HashSet<>(1));
  ...
  static void ensureNodeIdsAreSequential(ComputeGraph graph) {
    // TODO we could remove ID from the data model, and just rely on its index in the graph.
